...
public class ExportedJRPropertiesHandler implements IExportedResourceHandler, IPropertyCustomExporter {
	...
	public String getResourceNameExport() {
		try{
			...
			return "ezRepo Properties (" + props.size() + ")"; //$NON-NLS-1$
		} catch(Exception ex){
			ex.printStackTrace();
		}
		return "ezRepo Properties"; //$NON-NLS-1$
	}
	...
	public String getResourceNameImport(File exportedContainer) {
  	...
  	String result = "ezRepo Properties"; //$NON-NLS-1$
  	try{
	    is = new FileInputStream(f);
	    Properties loadedProperties = new Properties();
	    loadedProperties.load(is);
	    result = "ezRepo Properties (" + loadedProperties.size() + ")"; //$NON-NLS-1$
  	} catch (Exception ex){
  		ex.printStackTrace();
  	} finally {
	...
	public File exportContentFolder(List<IResourceDefinition> resourcesToExport) {
	    ...
	    props.store(out, "ezRepo Properties Backup"); //$NON-NLS-1$
		} catch (Exception ex){
			ex.printStackTrace();
			destDir = null;
		} finally {
