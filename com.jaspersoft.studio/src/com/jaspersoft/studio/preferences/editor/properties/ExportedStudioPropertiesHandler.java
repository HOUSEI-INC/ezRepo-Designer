...
public class ExportedStudioPropertiesHandler implements IExportedResourceHandler {
	...
	public String getResourceNameExport() {
		try{
			...
			return "ezRepo Designer Properties (" + size + ")"; //$NON-NLS-1$
		} catch(Exception ex){
			ex.printStackTrace();
		}
		return "ezRepo Designer Properties"; //$NON-NLS-1$
	}
	...
	public String getResourceNameImport(File exportedContainer) {
  	...
  	String result = "ezRepo Designer Properties"; //$NON-NLS-1$
  	try{
	    is = new FileInputStream(f);
	    Properties loadedProperties = new Properties();
	    loadedProperties.load(is);
	    result = "ezRepo Designer Properties (" + loadedProperties.size() + ")"; //$NON-NLS-1$
  	} catch (Exception ex){
  		ex.printStackTrace();
  	} finally {
	...
	public List<IResourceDefinition> getRestorableResources(File exportedContainer) {
		...
		if (destDir.exists() && destDir.list().length > 0){
			result.add(new BaseResource("All ezRepo Designer Preferences"));
		}
		return result;
	}
	...
	public List<IResourceDefinition> getExportableResources() {
		...
		try{
			...
			if ( props.size() > 0) result.add(new BaseResource("All ezRepo Designer Preferences"));
		} catch(Exception ex){
			ex.printStackTrace();
		}
		return result;
	...
	public File exportContentFolder(List<IResourceDefinition> resourcesToExport) {
		...
		if (!resourcesToExport.isEmpty()){
		    ...
		    props.store(out, "ezRepo Designer Properties Backup"); //$NON-NLS-1$
			} catch (Exception ex){
				ex.printStackTrace();
				destDir = null;
			} finally {
