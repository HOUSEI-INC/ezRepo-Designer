...
public final class PageFormatDialog extends FormDialog {
	...
	private void createColumns(Composite composite) {
		...
		cPrintOrder.setToolTipText(
				"Vertical垂直 - 印刷プロセスはまず上から下へ左から右へ1列目が完全に埋まってから、2列目、3列目と続きます。\n"
						+ "Horizontal水平 - 印刷プロセスはまず左から右へ上から下へ;1行目は任意の列に埋められ(埋まるとは限らない)、2行目、3行目と続きます。");
	...
	private void setJasperDesign(JasperDesign jd) {
		...
		if(StringUtils.isBlank(defunit)){
			defunit = "mm";
		}
		uw.setUnit(defunit);
