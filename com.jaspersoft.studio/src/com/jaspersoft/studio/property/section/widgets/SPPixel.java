...
public class SPPixel extends ASPropertyWidget<PixelPropertyDescriptor> {
	...
	private static String truncateAccuracyDouble(double number, int numDigits,String unit) {
		if (Unit.MM.equals(unit)) {
			return String.format("%." + 0 + "f",
					new BigDecimal(number).setScale(0, RoundingMode.CEILING).doubleValue());
		}
		if (Unit.CM.equals(unit)) {
			return String.format("%." + 2 + "f",
					new BigDecimal(number).setScale(2, RoundingMode.CEILING).doubleValue());
		}
		return String.format("%." + numDigits + "f",
				new BigDecimal(number).setScale(numDigits, RoundingMode.CEILING).doubleValue());
	}
...
//					.concat(" ".concat(defaultMeasure.getUnitName()))); //$NON-NLS-1$
		insertField.setText(truncateAccuracyDouble(dValue, defaultMeasure.getPrecision(),defaultMeasure.getKeyName())
				.concat(" ".concat(defaultMeasure.getUnitName()))); //$NON-NLS-1$
		else
