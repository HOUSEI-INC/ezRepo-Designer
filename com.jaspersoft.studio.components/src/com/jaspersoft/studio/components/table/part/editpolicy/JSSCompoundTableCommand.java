...
/**
 * Command used to set the property of one or more table cells. It will do the
 * autoresize of the table after executing the commands contained in this one,
 * if the flag is enabled. Also it can layout the whole table if set the flag
 * layoutTableContent to true.
 * 
 * Allow also to update the span of all the cells in the table when the command
 * is executed, undone o redone
 * 
 * Before to execute the inner commands store the width of every columns of the
 * table and restore if on the undo
 * 
...
public class JSSCompoundTableCommand extends JSSCompoundCommand {
	...
	/**
	 * This flag allow to layout the table content after the inner commands are
	 * executed
	 */
	 ...
	 * @param updateTableSpan if set to true will update the span of all the cells
	 *                        in the table when the command is executed, undone o
	 *                        redone
	 */
	...
	public void execute() {
		if (table.hasColumnsAutoresizeProportional()) {
			...
			if (!changed) {
				// The size was already right (probably because of the restoreColumnsSize) so
				// the cells was not
	...
	/**
	 * Iterate all the child recursively of the current group. If they are all in
	 * the exclusion set add also the group in the exclusion set
	 * 
	...
	protected void storeColumnsSize() {
		...
		originalColumnsSize = new HashMap<>();
	...
	protected HashSet<BaseColumn> getModifiedColumns() {
		HashSet<BaseColumn> result = new HashSet<>();
	...
	/**
	 * Set the layout table content flag. This flag allow to layout the table
	 * content after the inner commands are executed. By default this is false
	 * 
