...
public class ExtendedTemplateProvider implements TemplateProvider {
	...
	public List<TemplateBundle> getTemplateBundles() {
		...
		if (cache == null){
			...
			Enumeration<?> en = JaspersoftStudioPlugin.getInstance().getBundle().findEntries("templates/table", "*"+ FileExtension.PointJRXML, false); //Doesn't search in the subdirectories
		...
		for (String dir : pathsList) {
			File[] files = new File(dir).listFiles(new FileFilter() {
				...
				public boolean accept(File f) {
					return f.getName().endsWith( FileExtension.PointJRXML); //$NON-NLS-1$
				}
			});
